...
<%@page import="cn.zptc.entity.Visitor"%>
              ...
              <li class="sidebar-nav-link">
                ...
                <li class="sidebar-nav-link">
                    ...
                    <ul class="sidebar-nav sidebar-nav-sub" style="display: block;">
                        ...
                        <li class="sidebar-nav-link">
                            <a href="/Face-AI/vtransferServlet" class="sub-active">
                    ...
                    <div class="am-u-sm-12 am-u-md-12 am-u-lg-12">
                        <div class="widget am-cf">
                            <div class="widget-head am-cf">
                                <div class="widget-title  am-cf">访客</div>


                            </div>
                            <form action="/Face-AI/vqueryServlet">
                            <div class="widget-body  am-fr">
                                ...
                                <div class="am-u-sm-12 am-u-md-6 am-u-lg-3">
                                    <div class="am-form-group tpl-table-list-select">
                                        <select data-am-selected="{btnSize: 'sm'}" name="category">
							              ...
							              <option value="option3">业主</option>
							              <option value="option4">联系方式</option>
							              <option value="option5">时间</option>
							            </select>
                                    </div>
                                </div>
                                ...
                                <div class="am-u-sm-12">
                                    <table width="100%" class="am-table am-table-compact am-table-striped tpl-table-black " id="myTable">
                                        ...
                                        <tbody>
                                          	<%try{
                                          		Visitor[] visitorlist=(Visitor[]) request.getSession().getAttribute("visitor");
                                          		ArrayList idlist=(ArrayList) request.getSession().getAttribute("idlist");
	                                          	 ArrayList namelist=(ArrayList) request.getSession().getAttribute("namelist");
	                                             ArrayList telllist=(ArrayList) request.getSession().getAttribute("telllist");
	                                             ArrayList ownerlist=(ArrayList) request.getSession().getAttribute("ownerlist");
	                                             ArrayList datalist=(ArrayList) request.getSession().getAttribute("datalist");
	                                             for(int j=0;j<visitorlist.length;j++){	
                                          	%>
                                          			<%if((j+1)%2==1) {%>
									      				<tr class="gradeX">
									      					<td name="oid"><%=idlist.get(j) %></td>
			                                                <td name="oname"><%=namelist.get(j) %></td>
			                                                <td name="oowner"><%=ownerlist.get(j) %></td>
			                                                <td name="otell"><%=telllist.get(j) %></td>
			                                                <td name="odata"><%=datalist.get(j) %></td>
			                                                <td>
			                                                    <div class="tpl-table-black-operation">
			                                                        <a href="javascript:;" a class="edit" data-time="1">
...
$(".edit").click(function() {
	...
	if(b == "2") {
		//alert(b);
		$(this).parent().parent().parent().find("td:lt(5)").addClass('active');
		$(this).parent().parent().parent().find("td:nth-child(6) span").html('编辑');
		$(this).parent().parent().parent().find("td:lt(5)").attr("contenteditable", false);
		$(".edit").eq(index).attr("data-time", 1);
		var id = $(this).parent().parent().parent().find("td:nth-child(1)").html();
		var name = $(this).parent().parent().parent().find("td:nth-child(2)").html();
		var ownerid = $(this).parent().parent().parent().find("td:nth-child(3)").html();
		var tell = $(this).parent().parent().parent().find("td:nth-child(4)").html();
		var start = $(this).parent().parent().parent().find("td:nth-child(5)").html();
		$.ajax({
			type: "post",
			url: "/Face-AI/veditServlet",
			data: {
				"id":id,
				"name":name,
				"ownerid":ownerid,
		...
		//alert(b);
		console.log("2");
		$(this).parent().parent().parent().find("td:lt(5)").attr("contenteditable", true);
		$(this).parent().parent().parent().find("td:lt(1)").attr("contenteditable", false);
		$(this).parent().parent().parent().find("td:lt(5)").addClass('active');
		$(this).parent().parent()
		$(this).parent().parent().parent().find("td:nth-child(6) span").html('保存');
